...
*/
    ...
    if (begin >= 1) {
      ...
      feats += FeatureKey("Arg silly prior word  pos bigram", List(thisSentence.tokens(begin - 1).pos, token.pos, y)) -> 1.0
    }
    ...
    //                                Positional
    // -----------------------------------------------------------------------------
    //feats+= FeatureKey("absolute distance from candidate", List((Math.abs(token.index - x.parentIndex)).toString,y)) -> 1.0
  ...
  def myArgumentFeaturesNB(x: Candidate, y: Label): FeatureVector = {
    ...
    //feats += FeatureKey("Arg value of parent", List(parentToken.word, y)) -> 1.0
