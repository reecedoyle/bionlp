...
object Features {
  ...
  def myTriggerFeatures(x: Candidate, y: Label): FeatureVector = {
    ...
    feats += FeatureKey("Trig: mod deps count", List(mods.size.toString, y)) -> 1.0
    for (mod <- mods) {
      val mods2 = thisSentence.deps.filter(e => e.mod == mod.head).sortBy(d => d.label + d.head + d.mod)
      feats += FeatureKey("Trig: mod2 deps count", List(mods2.size.toString, y)) -> 1.0
      for (mod2 <- mods2) {
        val mods3 = thisSentence.deps.filter(e => e.mod == mod2.head).sortBy(d => d.label + d.head + d.mod)
        feats += FeatureKey("Trig: mod3 deps count", List(mods3.size.toString, y)) -> 1.0
        for (mod3 <- mods3) {
  ...
  def myArgumentFeatures(x: Candidate, y: Label): FeatureVector = {
    ...
    //                                Entity
    // -----------------------------------------------------------------------------
    feats += FeatureKey("Arg: Proteins in sentence", List(thisSentence.mentions.size.toString, y)) -> 1.0
