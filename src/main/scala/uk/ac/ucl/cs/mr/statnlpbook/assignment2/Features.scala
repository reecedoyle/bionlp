...
*/
    ...
    //                                 Syntax
    // -----------------------------------------------------------------------------
    feats += FeatureKey("Arg dependency between arg and source trigger", List(thisSentence.deps.filter(e => (e.head == begin && e.mod == parentToken.begin || e.mod == begin && e.head == parentToken.begin)).toString, y)) -> 1.0
  ...
  def myArgumentFeaturesNB(x: Candidate, y: Label): FeatureVector = {
    ...
    feats += FeatureKey("Arg absolute distance from candidate < 30", List((Math.abs(token.index - x.parentIndex) < 40).toString,y)) -> 1.0
