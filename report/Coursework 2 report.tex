...
As approximate precision was X, all labels suffered from errors in classification. The event label with the least misclassifications was phosphorylation while the model struggled with precision for regulation events the most. It was found that the most common types of errors were mislabelling a candidate trigger to be none. A concrete example of such an instance is shown in insert ref here.

The average precision for argument classification was Y. As with trigger classification, it was found that the main error was misclassifying candidate arguments to be of type none. An example instance of when this happened is shown in the appendix.

The frequency of these types of errors can be inferred from the following tables, where the columns represent the predicted label and the rows represent the true label:
...
As a result, even if the trained weights predict an illegal label, through constraints we can force classifications to a label (potentially the correct one) of a lower score. In the case of the earlier example, we found a significant increase in precision of labelling an argument as cause from X to Y.
